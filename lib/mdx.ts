...
export async function getAllBlogs(): Promise<BlogListItem[]> {
  console.log("=== getAllBlogs DEBUG START ===");
  const files = getBlogFiles();
  console.log("Files from getBlogFiles():", files);
  ...
  const blogs = await Promise.all(
    files.map(async (file) => {
      ...
      try {
        ...
        console.log("Parsed frontmatter status:", parsedFrontmatter.status);
  ...
  console.log("Blogs after processing:", blogs.length);
  console.log("Blogs array:", blogs);

  const filteredBlogs = blogs
    .filter((blog): blog is BlogListItem => blog !== null)
    .sort((a, b) => b.publishedAt.getTime() - a.publishedAt.getTime());

  console.log("Final filtered blogs:", filteredBlogs.length);
  console.log("=== getAllBlogs DEBUG END ===");
