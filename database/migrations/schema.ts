import { pgTable, foreignKey, text, timestamp, serial, integer, varchar, unique, boolean, date, primaryKey } from "drizzle-orm/pg-core"
...
export const users = pgTable("users", {
	...
	createdAt: timestamp("created_at", { mode: 'string' }).defaultNow(),
	provider: text(),
	jobRecommendationNotifPref: varchar("job_recommendation_notif_pref").default('enabled').notNull(),
	jobSubmissionNotifPref: varchar("job_submission_notif_pref").default('enabled').notNull(),
	apiToken: text("api_token"),
	apiTokenExpiration: timestamp("api_token_expiration", { mode: 'string' }),
	isNewUser: boolean("is_new_user").default(true).notNull(),
	jobSearchStatus: text("job_search_status").default('ready_to_interview').notNull(),
	emailVerified: timestamp({ mode: 'string' }),
	isEmailVerified: boolean("is_email_verified").default(false).notNull(),
	verificationCode: text("verification_code"),
	verificationCodeExpires: timestamp("verification_code_expires", { mode: 'string' }),
}, (table) => [
