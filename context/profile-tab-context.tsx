...
export type TabName = "Profile" | "Files" | "Verification";
...
export function ProfileTabProvider({ children }: { children: ReactNode }) {
  ...
  const [stepCompletionStatus, setStepCompletionStatus] = useState<StepCompletionStatus>({
    Profile: false,
    Files: false,
    Verification: true, // Always true since it's just a submission step
    Review: false
  });
  ...
  const validateProfileStep = (data: IProfileResponse): boolean => {
    ...
    const hasRequiredFields = requiredProfileFields.every(field => field && field.trim() !== '');
    ...
    return hasRequiredFields && hasPhones && hasEmails && hasAssessmentOrContent;
  };
  ...
  const validateStepCompletion = useCallback(async (step: TabName): Promise<boolean> => {
  ...
  const isStepAccessible = useCallback((step: TabName): boolean => {
    switch (step) {
      case "Profile":
  ...
  const refreshStepCompletion = useCallback(async () => {
    ...
    const verificationComplete = true; // Always true since it's just a submission step

    setStepCompletionStatus({
      Profile: profileComplete,
      Files: filesComplete,
      Verification: verificationComplete,
      Review: false // Remove Review step
    });
